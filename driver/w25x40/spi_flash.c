...
{ 
  ...
  SPI_Cmd(SPI2, ENABLE);
...
#else
	GPIO_InitTypeDef GPIO_InitStructure;  
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_NoJTRST, ENABLE);//关闭PB4的JNTRST功能
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//关闭PB3和PA15的JTAG功能
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(SPI_FLASH_SCK_BASE, &GPIO_InitStructure);
	
    // SPI_FLASH_MOSI_S PB4
  GPIO_InitStructure.GPIO_Pin = SPI_FLASH_MOSI_PIN;
	GPIO_Init(SPI_FLASH_MOSI_BASE, &GPIO_InitStructure);

    // SPI_FLASH_NSS_S  PA11
  GPIO_InitStructure.GPIO_Pin = SPI_FLASH_NSS_PIN;
	GPIO_Init(SPI_FLASH_NSS_BASE, &GPIO_InitStructure);
    
	// SPI_FLASH_MISO_S PA12
  GPIO_InitStructure.GPIO_Pin = SPI_FLASH_MISO_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(SPI_FLASH_MISO_BASE, &GPIO_InitStructure);
...
void SPI_FLASH_SendByte(u8 byte)
{
    ...
    for(i=0; i<8; i++)
    {
        ...
        else
        {
            ...
            SPI_FLASH_SCK_S_0;     //toggle clock high
            byte <<= 1;
